...
public class PasskeymeSDK {
    ...
    public func passkeyRegister(challenge: RegisterChallenge, completion: @escaping (Result<String, Error>) -> Void) {
...
extension PasskeysSDK: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    public func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        if let credential = authorization.credential as? ASAuthorizationPlatformPublicKeyCredentialRegistration {
            ...
            if let decodedChallenge = updateClientDataJSONChallenge(from: credential.rawClientDataJSON) {
                ...
                completion(.failure(NSError(domain: "PasskeysSDK", code: -1, userInfo: [NSLocalizedDescriptionKey: "Challenge decode conversion Fail."])))
            }
            ...
            if let rawAuthenticatorData = credential.rawAuthenticatorData,
               ...
               let userID = credential.userID {
                ...
                completion(.failure(NSError(domain: "PasskeysSDK", code: -1, userInfo: [NSLocalizedDescriptionKey: "Credential decode conversion Fail."])))
            }
        } else {
            completion(.failure(NSError(domain: "PasskeysSDK", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unknown credential type."])))
        }
    }
